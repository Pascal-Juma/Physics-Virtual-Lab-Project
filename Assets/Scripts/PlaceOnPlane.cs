...
{
    [Header("References")]
    public GameObject objectToPlace;          // Pendulum prefab
    public ARRaycastManager raycastManager;   // ARRaycastManager

    private List<ARRaycastHit> hits = new List<ARRaycastHit>();
    private GameObject spawnedObject;         // Reference to placed object
    private bool isSelected = false;          // <-- NEW
    ...
    // ----------------------------
    // 1. PLACE OBJECT (only if not spawned OR not clicked on object)
    // ----------------------------
    ...
    void TryPlaceObject(Vector2 screenPos)
    {
        if (raycastManager.Raycast(screenPos, hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            if (spawnedObject == null)
            {
                ...
                // Connect Pendulum to UI
                Pendulum3D pend = spawnedObject.GetComponentInChildren<Pendulum3D>();
                PendulumUI ui = Object.FindFirstObjectByType<PendulumUI>();
                if (ui != null && pend != null)
                    ui.SetPendulum(pend);
            }
            else if (!isSelected)
            {
